...
from app.utils.hash_ids_constants import (
    DEFAULT_ALPHABET,
    DEFAULT_BLOCKLIST,
    DEFAULT_MIN_LENGTH,
    _exact_match,
...
class ManualHashids:
    ...
    def __encode_numbers(self, numbers: Sequence[int], increment: int = 0) -> str:
        ...
        offset = sum((ord(self.__alphabet[v % len(self.__alphabet)]) + i for i, v in enumerate(numbers)))
    ...
    def __is_blocked_id(self, id_: str) -> bool:
        ...
        if id_.startswith(self.__blocklist_match_at_ends) or id_.endswith(self.__blocklist_match_at_ends):
            return True
